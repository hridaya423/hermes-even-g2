...
class ControlService:
    def __init__(self, store: Store, hermes: HermesClient, action_max_age_seconds: int):
        self.store = store
        self.hermes = hermes
    ...
    async def execute(self, action: AgentAction, device: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind == ActionKind.CREATE_SESSION:
            return await self.hermes.create_session(action.payload)
    ...
    async def _run_prompt(self, session_id: str, text: str, device_id: str, options: dict | None) -> None:
        ...
        try:
            ...
            await self.store.audit(device_id, "prompt", session_id, run_id, "completed")
        except Exception as error:
