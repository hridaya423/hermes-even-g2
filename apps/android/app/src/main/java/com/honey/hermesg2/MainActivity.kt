...
class MainActivity : ComponentActivity(), TextToSpeech.OnInitListener {
    ...
    @Composable private fun Controller(deepLinkSession: String?) {
        ...
        LaunchedEffect(tab) { auxiliary = when (tab) { "Jobs" -> { jobs = if (snapshot?.hermes?.jobs == true) runCatching { client.jobs() }.getOrElse { error = it.message; null } else null; if (snapshot?.hermes?.jobs == true) "" else "Jobs are not advertised by this Hermes build." }; "Models" -> if (snapshot?.hermes?.models == true) runCatching { client.models() }.getOrElse { it.message.orEmpty() } else "Model options are unavailable."; "Skills" -> if (snapshot?.hermes?.skills == true) runCatching { client.skills() }.getOrElse { it.message.orEmpty() } else "Skills are not advertised by this Hermes build."; "Security" -> { devices = runCatching { client.devices() }.getOrElse { error = it.message; null }; runCatching { client.audit() }.getOrElse { it.message.orEmpty() } }; else -> "" } }
